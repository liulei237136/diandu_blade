 <div class="tw-flex tw-items-center">
     <a href="{{ $repository->link() }}"
         class="tw-no-underline tw-px-4 tw-py-2 tw-flex tw-items-center tw-text-black tw-border-b
        {{ if_route('repositories.show') ? 'tw-border-b-red-500 ' : '' }}
     ...
     <a href="{{ route('repository_audio.show', ['repository' => $repository]) }}"
         class="
        {{ if_route('repository_audio.show') || if_route('repository_audio.edit')? 'tw-border-b-red-500': '' }}
        tw-border-b tw-no-underline tw-px-4 tw-py-2 tw-flex tw-items-center  hover:tw-border-b hover:tw-border-b-gray-200">
         <x-icon name="audio" class="tw-hidden md:tw-block tw-w-4 tw-h-4 tw-mr-1" />
         <span>包含音频</span>
     </a>
     <a href="{{ $repository->link('repository_comments.show') }}"
         class="{{ if_route('repository_comments.show') ? ' tw-border-b-red-500' : '' }} tw-border-b  tw-no-underline  tw-px-4 tw-py-2 tw-flex tw-items-center tw-text-black hover:tw-text-black  hover:tw-border-b hover:tw-border-b-gray-200">
     ...
     <a href="{{ $repository->link('repository-downloads.index') }}"
         class="{{ if_route('repository-downloads.index') ? ' tw-border-b-red-500' : '' }} tw-border-b  tw-no-underline  tw-px-4 tw-py-2 sm:tw-flex tw-items-center tw-text-black hover:tw-text-black  hover:tw-border-b hover:tw-border-b-gray-200">
     ...
     @can('update', $repository)
         <a href="{{ $repository->link('repository_setting.show') }}"
             class="{{ if_route('repository_setting.show') ? ' tw-border-b-red-500' : '' }} tw-border-b  tw-hidden tw-no-underline  tw-px-4 tw-py-2 sm:tw-flex tw-items-center tw-text-black hover:tw-text-black  hover:tw-border-b hover:tw-border-b-gray-200">
